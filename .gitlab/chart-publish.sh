...
###
### Original script: https://github.com/stefanprodan/helm-gh-pages/blob/master/src/entrypoint.sh
### Some GitLab-friendly adjustments have been made by Wallarm
###
...
CHARTS_DIR=${CHARTS_DIR:-$2}
CHARTS_URL=${CHARTS_URL:-$3}
OWNER=${OWNER:-$4}
REPOSITORY=${REPOSITORY:-$5}
BRANCH=${BRANCH:-$6}
TARGET_DIR=${TARGET_DIR:-$7}
HELM_VERSION=${HELM_VERSION:-$8}
LINTING=${LINTING:-$9}
COMMIT_USERNAME=${COMMIT_USERNAME:-${10}}
COMMIT_EMAIL=${COMMIT_EMAIL:-${11}}
APP_VERSION=${APP_VERSION:-${12}}
CHART_VERSION=${CHART_VERSION:-${13}}
INDEX_DIR=${INDEX_DIR:-${14}}
ENTERPRISE_URL=${ENTERPRISE_URL:-${15}}
...
main() {
  ...
  if [[ -z "$COMMIT_USERNAME" ]]; then
      COMMIT_USERNAME="${GITLAB_USER_LOGIN}"
  fi
